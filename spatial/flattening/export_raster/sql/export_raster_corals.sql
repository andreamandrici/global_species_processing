...
SELECT qid,cid,geom
FROM species_2022_all_taxa.corals_flat ---<---- DEFINE IT HERE!
--WHERE corals && '{59468,136135}'
; ---<---- FILTER IT HERE!
----------------------------------------------------------
DROP TABLE IF EXISTS flat_attributes;CREATE TEMPORARY TABLE flat_attributes AS
SELECT cid,
corals_richness ncid ---<---- DEFINE IT HERE!
FROM species_2022_all_taxa.corals_attributes ---<---- DEFINE IT HERE!
;
-- threatened
DROP TABLE IF EXISTS flat_attributes;CREATE TEMPORARY TABLE flat_attributes AS
SELECT cid,
corals_threatened_richness ncid ---<---- DEFINE IT HERE!
FROM species_2022_all_taxa.corals_attributes ---<---- DEFINE IT HERE!
WHERE corals_threatened_richness IS NOT NULL ---<---- DEFINE IT HERE!
; 
--- endemic
DROP TABLE IF EXISTS flat_attributes;CREATE TEMPORARY TABLE flat_attributes AS
SELECT cid,
corals_endemic_richness ncid ---<---- DEFINE IT HERE!
FROM species_2022_all_taxa.corals_attributes ---<---- DEFINE IT HERE!
WHERE corals_endemic_richness IS NOT NULL ---<---- DEFINE IT HERE!
;
--- threatened_endemic
DROP TABLE IF EXISTS flat_attributes;CREATE TEMPORARY TABLE flat_attributes AS
SELECT cid,
corals_threatened_endemic_richness ncid ---<---- DEFINE IT HERE!
FROM species_2022_all_taxa.corals_attributes ---<---- DEFINE IT HERE!
WHERE corals_threatened_endemic_richness IS NOT NULL ---<---- DEFINE IT HERE!
;
----------------------------------------------------------
