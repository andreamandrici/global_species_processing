...
#./a_input_birds.sh ${ncores} > logs/a_input_birds_log.txt 2>&1 wait

# B-CLIP TABLES
## populates clip tables
./b_clip_birds.sh ${ncores} > logs/b_clip_birds_log.txt 2>&1 wait

# C-RASTER TABLES
## populates raster tables
./c_rast_birds.sh ${ncores} > logs/c_raster_birds_log.txt 2>&1 wait

# D-TILED TABLES
## DA-populates tiled tables (in parallel)
./da_tiled_birds.sh ${ncores} > logs/da_tiled_birds_log.txt 2>&1 wait
