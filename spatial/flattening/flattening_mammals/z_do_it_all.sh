...
#./a_input_mammals.sh ${ncores} > logs/a_input_mammals_log.txt 2>&1 wait

# B-CLIP TABLES
## populates clip tables
./b_clip_mammals.sh ${ncores} > logs/b_clip_mammals_log.txt 2>&1 wait

# C-RASTER TABLES
## populates raster tables
./c_rast_mammals.sh ${ncores} > logs/c_raster_mammals_log.txt 2>&1 wait

# D-TILED TABLES
## DA-populates tiled tables (in parallel)
./da_tiled_mammals.sh ${ncores} > logs/da_tiled_mammals_log.txt 2>&1 wait
