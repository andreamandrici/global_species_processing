...
DROP MATERIALIZED VIEW IF EXISTS dopa.dopa_species_list_template CASCADE;
CREATE MATERIALIZED VIEW dopa.dopa_species_list_template AS
SELECT * FROM dopa.dopa_species LIMIT 0;

-------FN_GET_LIST_SPECIES_OUTPUT---------------------------------------
DROP FUNCTION IF EXISTS dopa.get_dopa_species_list(bigint, text, text, text, text, text, text, boolean, text, text, text, boolean, text, text, text, text, text);
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list(
...
RETURNS SETOF dopa.dopa_species
LANGUAGE 'plpgsql'
AS $BODY$
DECLARE
...
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list(bigint, text, text, text, text, text, text, boolean, text, text, text, boolean, text, text, text, text, text)
IS '
...
a AS (
SELECT
id_no,class,order_,family,genus,binomial,category,threatened,country_n,endemic,t.*
FROM dopa.dopa_species, UNNEST(ecosystems,habitats,country,stresses,threats,research_needed,conservation_needed,usetrade) t(ecosystems,habitats,country,stresses,threats,research_needed,conservation_needed,usetrade)';
IF a_id_no IS NOT NULL THEN sql := sql || ' WHERE id_no = '||a_id_no||' ';
ELSE	sql := sql || ' WHERE id_no = 60354712 ';
END IF;
sql := sql || '
)
SELECT
a.id_no,a.class,a.order_,a.family,a.genus,a.binomial,a.category,a.threatened,a.country_n,a.endemic,a.ecosystems,
...
FROM a
LEFT JOIN dopa.class_species_habitat b ON a.habitats=b.code
LEFT JOIN dopa.class_species_country c ON a.country=c.code
LEFT JOIN dopa.class_species_stress d ON a.stresses=d.code
LEFT JOIN dopa.class_species_threat e ON a.threats=e.code
LEFT JOIN dopa.class_species_research_needed f ON a.research_needed=f.code
LEFT JOIN dopa.class_species_conservation_needed g ON a.conservation_needed=g.code
LEFT JOIN dopa.class_species_usetrade h ON a.usetrade=h.code
...
ALTER FUNCTION dopa.get_dopa_species(bigint)
    OWNER TO h05ibex;

COMMENT ON FUNCTION dopa.get_dopa_species(bigint)
    IS 'Shows for a single species direct and related detailed attributes';
    
-------FN_GET_LIST_CATEGORIES--------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_category()
RETURNS SETOF dopa.class_species_category
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_category;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_category() IS 'Shows list of EXISTING categories';


-------FN_GET_LIST_CONSERVATION_NEEDED--------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_conservation_needed()
RETURNS SETOF dopa.class_species_conservation_needed
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_conservation_needed;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_conservation_needed() IS 'Shows list of EXISTING conservation needed';

-------FN_GET_LIST_COUNTRIES --------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_country()
RETURNS SETOF dopa.class_species_country
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_country;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_country() IS 'Shows list of EXISTING countries';

-------FN_GET_LIST_HABITATS --------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_habitat()
RETURNS SETOF dopa.class_species_habitat
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_habitat;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_habitat() IS 'Shows list of EXISTING habitats;';

-------FN_GET_LIST_RESEARCH_NEEDED--------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_research_needed()
RETURNS SETOF dopa.class_species_research_needed
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_research_needed;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_research_needed() IS 'Shows list of EXISTING research needed';


-------FN_GET_LIST_STRESSES--------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_stress()
RETURNS SETOF dopa.class_species_stress
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_stress;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_stress() IS 'Shows list of EXISTING stresses';

-------FN_GET_LIST_THREATS--------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_threat()
RETURNS SETOF dopa.class_species_threat
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_threat;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_threat() IS 'Shows list of EXISTING threats';

-------FN_GET_LIST_USETRADE--------------------------------
CREATE OR REPLACE FUNCTION dopa.get_dopa_species_list_usetrade()
RETURNS SETOF dopa.class_species_usetrade
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
RETURN QUERY EXECUTE 'SELECT * FROM dopa.class_species_usetrade;';
END;
$BODY$;
COMMENT ON FUNCTION dopa.get_dopa_species_list_usetrade() IS 'Shows list of EXISTING uses and trades';


--------- GRANTS FOR DOPA REST -------------------------------------------------------------
GRANT USAGE ON SCHEMA dopa TO h05ibexro;
GRANT SELECT ON ALL TABLES IN SCHEMA dopa TO h05ibexro;
